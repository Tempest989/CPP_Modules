...
Form::Form() : name("Default"), sign_min(150), sign_exe(150)
{
	std::cout << "Form Default constructor called" << std::endl;
	this->been_signed = false;
}

Form::Form(const Form &input) : name(input.getName()), sign_min(input.getSignMin()), sign_exe(input.getSignExecute())
{
	std::cout << "Form Copy constructor called" << std::endl;
	*this = input;
}

Form::Form(std::string const input, unsigned int min, unsigned int exe) : name(input), sign_min(min), sign_exe(exe)
{
...
{
	out << input.getName() << ", form grade minimum " << input.getSignMin() << ", form grade execute " << input.getSignExecute() << ", form been signed " << input.getBeenSign() << "." << std::endl;
	return (out);
}
